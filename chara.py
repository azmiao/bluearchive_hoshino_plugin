...
try:
    ...
    unknown_chara_icon = R.img(f'bluearchive/unit/icon_unit_{UNKNOWN}11.png').open()
...
class Roster:
    ...
    def update(self):
        importlib.reload(_char_data)
        self._roster.clear()
        for idx, names in _char_data.CHARA_NAME.items():
            for n in names:
...
class Chara:
    ...
    @property
    def name(self):
        return _char_data.CHARA_NAME[self.id][0] if self.id in _char_data.CHARA_NAME else _char_data.CHARA_NAME[UNKNOWN][0]
    ...
    def icon(self):
        ...
        res = R.img(f'bluearchive/unit/icon_unit_{self.id}{star}1.png')
        if not res.exist:
            res = R.img(f'bluearchive/unit/icon_unit_{self.id}31.png')
        if not res.exist:
            res = R.img(f'bluearchive/unit/icon_unit_{self.id}11.png')
        if not res.exist:
            res = R.img(f'bluearchive/unit/icon_unit_{self.id}{star}1.png')
        if not res.exist:
            res = R.img(f'bluearchive/unit/icon_unit_{self.id}31.png')
        if not res.exist:
            res = R.img(f'bluearchive/unit/icon_unit_{self.id}11.png')
        if not res.exist:
            res = R.img(f'bluearchive/unit/icon_unit_{UNKNOWN}31.png')
        return res
